...
// The contextSetUser() method returns a new copy of the request with the provided User struct
...
// The contextSetUser() retrieves the User struct from the request context. The only time that we'll
// use this helper is when we logically expect there to be User struct value in the context, and
// if it doesn't exist it will firmly be an 'unexpected' error.
// As we discussed erlier in the book, its ok to panic in those circumstances.
